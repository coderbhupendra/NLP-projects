'''to randomly shuffle the wrong sentences and correct sentences'''
import os
import sys
import gzip
import time
...
def create_data():
	...
	for i in range(len(data_copy)/3):
		line=data_copy[i]
		if(len(line)==0):
			print "zero"
		else:	
...
with open("data/data_no_new_zip.mdl", "wb") as m:
		pickle.dump([data_no_new_x[:int(size*.8)],data_no_new_y[:int(size*.8)]], m)
#make test set 
print"dumped train set"
with open("data/data_no_test_zip.mdl", "wb") as m:
		pickle.dump([data_no_new_x[int(size*.8):size],data_no_new_y[int(size*.8):size]], m)
